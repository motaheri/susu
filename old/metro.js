...
MetroScroller.initButtons = function(){
	...
	$('.msl-metro-page .article .overlay').unbind('hover');
	$('.msl-metro-page .article img').unbind('hover');

	$('.msl-metro-page .article .overlay').hover(function(){
		$('.msl-metro-page .overlay').fadeIn('fast');
		$(this).fadeOut('fast');
		//$('.msl-metro-page .overlay').fadeIn('fast');
	}, function(){
		//$(this).show();
	...
	$('.msl-metro-page .article img').hover(function(){
	}, function(){
		$('.msl-metro-page .overlay').fadeIn('fast');
	});
...
MetroScroller.loadData = function(pages){
	...
	for (var i = 0; i < pages.length; i++) {
		...
		for (var i2 = 0; i2 < page.length; i2++) {
			...
			articlehtml = articlehtml + '<a href="#"><img src="' + article['img'] + '" /><div class="overlay"></div></a>';
			articlehtml = articlehtml + '<div class="heading"><span class="title">' + article['title'] + '<span class="category"> | ' + article['category'] + '</span></span><br /><span class="featured-n">' + article['text'] + '</span></div>';
			...
			articlehtml = '<div class="article article-' + articleID + '">' + articlehtml + '</div>';
...
MetroScroller.deSaturate = function(){
	...
	$('.msl-metro-page-hidden .article img').each(function(){
		...
		$(this).clone().insertAfter(this).hide();
	...
	$('.msl-metro-page-selected a img').css('filter','');
};
